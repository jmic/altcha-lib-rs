...
use actix_web::{get, post, App, HttpServer, web, HttpResponse, ResponseError, body, http};
use base64::Engine;
use base64::prelude::BASE64_STANDARD;
use altcha_lib_rs::{Challenge, ChallengeOptions, error};
...
fn status_code_into_u16<S>(status_code: &http::StatusCode, serializer: S) -> Result<S::Ok, S::Error> where S: Serializer {
...
async fn get_challenge() -> actix_web::Result<web::Json<Challenge>, ErrorResponse> {
    let res = altcha_lib_rs::create_challenge(ChallengeOptions{hmac_key: SECRET_KEY, expires: Some(Utc::now()+chrono::TimeDelta::minutes(5)), number: Some(22222), max_number: Some(100),..Default::default()})?;
    Ok(web::Json(res))
}

#[post("/submit")]
async fn verify(req: web::Form<SubmitRequest>) -> actix_web::Result<web::Json<VerifiedResponse>, ErrorResponse> {
